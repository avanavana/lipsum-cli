...
register_case 'TC61' 'Sources' 'The sources action lists the available bundled and configured source corpora.' "HOME=\"\$(mktemp -d)\" LIPSUM_SOURCE_DIR='$project_dir/share/sources' LIPSUM_DICT='$project_dir/share/lorem.words' $lp sources"
...
register_case 'TC63' 'Sources' 'The short source option selects another named source corpus.' "HOME=\"\$(mktemp -d)\" LIPSUM_SOURCE_DIR='$project_dir/share/sources' LIPSUM_DICT='$project_dir/share/lorem.words' $lp -s de 6 words -n -l | tr ' ' '\n' | awk 'NR==FNR { for (i = 1; i <= NF; i++) seen[\$i]=1; next } NF { if (!seen[\$1]) bad=1 } END { exit bad }' '$project_dir/share/sources/de.words' -"
register_case 'TC64' 'Sources' 'Config can change the default source for bare and explicit generation.' "mkdir -p '$artifacts'; printf \"default_source='hipster'\n\" > '$artifacts/source-config.zsh'; LIPSUM_CONFIG='$artifacts/source-config.zsh' LIPSUM_SOURCE_DIR='$project_dir/share/sources' LIPSUM_DICT='$project_dir/share/lorem.words' $lp 6 words -n -l | tr ' ' '\n' | awk 'NR==FNR { for (i = 1; i <= NF; i++) seen[\$i]=1; next } NF { if (!seen[\$1]) bad=1 } END { exit bad }' '$project_dir/share/sources/hipster.words' -"
