...
register_case 'TC64A' 'Sources' 'A specific source can be inspected through the sources action.' "out=\"\$(HOME=\"\$(mktemp -d)\" LIPSUM_SOURCE_DIR='$project_dir/share/sources' LIPSUM_DICT='$project_dir/share/lorem.words' $lp sources corporate)\"; printf '%s\n' \"\$out\"; printf '%s\n' \"\$out\" | grep -F 'Source Details' >/dev/null && printf '%s\n' \"\$out\" | grep -F 'Title: Corporate Ipsum' >/dev/null && printf '%s\n' \"\$out\" | grep -F 'Slug: corporate' >/dev/null && printf '%s\n' \"\$out\" | grep -F 'Type: built-in' >/dev/null && printf '%s\n' \"\$out\" | grep -F 'Sample:' >/dev/null"
