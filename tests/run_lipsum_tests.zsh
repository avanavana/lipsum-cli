...
register_case 'TC86' 'Templates' 'A built-in template can render a single item.' "$lp template notification"
register_case 'TC87' 'Templates' 'A top-level count works before the template command.' "$lp 3 template conventional-commit -p none | awk 'END { print NR; if (NR != 3) exit 1 }'"
register_case 'TC88' 'Templates' 'The templates action separates built-in and imported templates and includes sample output.' "tmp_home=\"\$(mktemp -d)\"; mkdir -p \"\$tmp_home/.lipsum/templates\"; printf '# title: Custom Ticket\nticket-{{number(100-999)}} {{choice(alpha|beta|gamma)}}\n' > \"\$tmp_home/.lipsum/templates/custom-ticket.tpl\"; out=\"\$(HOME=\"\$tmp_home\" $lp templates)\"; printf '%s\n' \"\$out\"; printf '%s\n' \"\$out\" | grep -F 'Built-In Templates:' >/dev/null && printf '%s\n' \"\$out\" | grep -F 'Imported Templates:' >/dev/null && printf '%s\n' \"\$out\" | grep -F 'Conventional Commit (conventional-commit)' >/dev/null && printf '%s\n' \"\$out\" | grep -F 'Custom Ticket (custom-ticket)' >/dev/null; rc=\$?; rm -rf \"\$tmp_home\"; exit \$rc"
...
register_case 'TC90' 'Templates' 'Templates also render cleanly through JSON output formatting.' "out=\"\$($lp 2 template conventional-commit -p none -f json)\"; printf '%s\n' \"\$out\"; printf '%s\n' \"\$out\" | grep -Eq '^\\[ \".+\", \".+\" \\]$'"
